...
class Test_fixturn_class():

    @allure.title("登录测试003")
    ...
    @allure.title("加入购物测试002")
    def test_addshopcar002(self, get_token_by_gcs):
        # 从字典中获得token
        with allure.step('从字典中获得token'):
            token = GlobalContext().get_value('token')
        # TODO    1: 确定请求数据，发送请求
    ...
    @allure.title("加入购物测试003")
    def test_addshopcar003(self, get_token_by_gcs):
        # 从字典中获得token
        with allure.step('从字典中获得token'):
            token = GlobalContext().get_value('token')
        # TODO    1: 确定请求数据，发送请求
